    ...
    const steps = [
        {
            ...
            description:
            'Set up Conan Chat Logger in a few quick steps.',
        },
        {
            id: 'save-chats-to-folder',
            title: 'Save chats to a folder',
            description:
                "Choose where chats should be stored.",
            content: (
                <div>
                    <Button variant="outline" onClick={() => window.api.showOpenDialog({ title: 'Select a folder', properties: ['openDirectory', 'openDirectory'] }).then(result => {
                        if (result.filePaths.length > 0) {
                            setSettings({ ...settings!, dataDir: result.filePaths[0] });
                        }
                    })}>
                        Browse: {settings?.dataDir ? settings.dataDir : 'No folder selected'}
                    </Button>                
                </div>
            ),
        },
            ...
            content: (
                <div className="space-y-4 text-sm">
                    <ol className="list-decimal space-y-4 pl-5">
                        ...
                        <li>
                            <p>
                                Check Enable Webhook and set the Webhook URL to: <code>http://localhost:{settings.port}/conan</code>
                            </p>
                            <img src={webhookScreenshot} alt="Example webhook setup details" className="mt-2 w-3/4 max-w-3xl rounded-md border" />
                        </li>
                    </ol>
                </div>
            ),
        },
        {
            ...
            content: (
                <Field>
                    ...
                    <Input id="session-timeout" value={sessionTimeout} aria-invalid={!isNumberValid} onChange={(e) => {
                        const validatedValue = Number(e.target.value);
                        console.log(validatedValue);
                        const isValid = !isNaN(validatedValue) && validatedValue > 0;
            ...
            <Card className="mx-auto w-fit min-w-sm max-w-full">
                ...
                <CardFooter> 
                    <div className="flex items-center w-full">
                        {activeStep > 0 && (
                            <Button className="mr-auto" variant="outline" onClick={() => setActiveStep(activeStep - 1)}>
                                Previous Step
                            </Button>
                        )}
                        {activeStep < steps.length - 1 && (
                            <Button className="ml-auto" variant="default" onClick={() => setActiveStep(activeStep + 1)}>
                                Next Step
                            </Button>
                        )}
                        {activeStep === steps.length - 1 && (
                            <Button
                                ...
                                onClick={() => {
                                    ...
                                    const newSettings ={ ...settings!, onboardingCompleted: true, sessionGapMinutes: Number(sessionTimeout) };
